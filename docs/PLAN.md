...
- **Task 1.1** Init Tauri v2 + React + TypeScript + Vite project → `subagent`
- **Task 1.2** Configure Tailwind, shadcn/ui, Zustand, React Query, `@/` alias → `subagent`
- **Task 1.3** Initialize SQLite schema with migrations → `subagent`
- **Task 1.4** Tauri Rust CRUD commands for all entities → `subagent`
> **Phase Exit Criteria:** Merge `phase/1-infrastructure` into `main` via non-fast-forward (`--no-ff`), tag `v0.1-infra` (optional), and push to GitHub.
...
> **Phase Exit Criteria:** git checkout main && git merge phase/2-core-layers --no-ff && git tag -a v0.2-core -m "Phase 2 Complete: Core App Shell & Data layer ready" && git push origin main --tags
...
> **Phase Exit Criteria:** git checkout main && git merge phase/3-management --no-ff && git tag -a v0.3-mgmt -m "Phase 3 Complete: Sprints and Team management views" && git push origin main --tags
...
- **Task 4.4** File attachments → `subagent`
- **Task 4.5** Time tracking → `skill:frontend-design`
> **Phase Exit Criteria:** git checkout main && git merge phase/4-task-engine --no-ff && git tag -a v0.4-tasks -m "Phase 4 Complete: Core task CRUD, commenting, and file mechanics" && git push origin main --tags

### Phase 5 — Board Views
> **Git Setup:** git checkout main && git pull && git checkout -b phase/5-boards
- **Task 5.1** User Story Board → `skill:frontend-design`
- **Task 5.2** Task Board → `skill:frontend-design` + `subagent` (DnD)
- **Task 5.3** Discovery Board → `skill:frontend-design`
- **Task 5.4** Sprint Planning Board → `skill:frontend-design`
- **Task 5.5** Drag & drop between columns → `subagent`
> **Phase Exit Criteria:** git checkout main && git merge phase/5-boards --no-ff && git tag -a v0.5-boards -m "Phase 5 Complete: Interactive agile board views with DnD tracking" && git push origin main --tags

### Phase 6 — Roadmap
> **Git Setup:** git checkout main && git pull && git checkout -b phase/6-roadmap
- **Task 6.1** Gantt-style roadmap → `subagent`
- **Task 6.2** Draggable bars + today indicator → `subagent`
> **Phase Exit Criteria:** git checkout main && git merge phase/6-roadmap --no-ff && git tag -a v0.6-roadmap -m "Phase 6 Complete: Gantt roadmap timeline visualizer" && git push origin main --tags
...
> **Phase Exit Criteria:** git checkout main && git merge phase/7-analytics --no-ff && git tag -a v0.7-reports -m "Phase 7 Complete: Burndown and analytics reporting dashboards" && git push origin main --tags
...
- **Task 8.2** Native OS notifications → `subagent`
- **Task 8.3** Notification bell badge → `skill:frontend-design`
> **Phase Exit Criteria:** git checkout main && git merge phase/8-notifications --no-ff && git tag -a v0.8-alerts -m "Phase 8 Complete: Notification center and native OS toast messaging" && git push origin main --tags
...
- **Task 9.4** App icon, window title, about page → `subagent`
> **Phase Exit Criteria:** git checkout main && git merge phase/9-polish --no-ff && git tag -a v1.0.0 -m "FlowBoard MVP Launch Release" && git push origin main --tags
...
**Assigned to:** skill:frontend-design + subagent (backend DnD support)  
**Depends on:** Phase 4
